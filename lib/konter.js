...
function emitApiActivity(type) {
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('konter:api-activity', { detail: { type } }))
  }
}
...
export async function api(path, { method = 'GET', body, token } = {}) {
  ...
  if (isMutation) emitApiActivity('start')
  ...
  try {
    ...
    if (isMutation) emitApiActivity('end')
  }
}
