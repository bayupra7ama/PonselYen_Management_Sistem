// ESC/POS receipt generator for 58mm (32 cols) / 80mm (48 cols) thermal printers.
...
export function widthCols(width) { return String(width) === '58' ? 32 : 48 }

// Build service receipt (nota service) bytes
export function buildServiceReceipt(service, settings = {}, width = '58') {
  const cols = widthCols(width)
  const s = service || {}
...
export function buildSaleReceipt(sale, settings = {}, width = '58') {
  const cols = widthCols(width)
  const s = sale || {}
  const b = new Builder(cols)
  b.init()
